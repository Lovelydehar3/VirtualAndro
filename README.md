...
This is not a full emulator or hardware VM. Android does not let a normal third-party app securely run arbitrary APKs inside its own process with zero host risk. The closest supported no-root model is a managed work profile, where Android separates apps, app data, accounts, storage, and many cross-profile sharing paths from the personal profile.
